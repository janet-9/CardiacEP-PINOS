'''
Script for comparing the training, evaluation and compute time for each of the neural operator model. 
...
# Extract the PINO model type: 1 = fixed, 2 = softadapt, 3 = reladapt.  
def extract_pino_type(folder_name):
    """
    Extract the numeric value from a folder name like 'PINO_1.0'.
    Returns None if pattern doesn't match.
    """
    #match = re.search(r'PINO_([\d\.]+)', folder_name)
    match = re.search(r'PINO_([\d\.]+)_finite', folder_name)
    if match:
...
def make_label(folder: str, eval_metric: str) -> str:
    ...
    if "data" in f:
        ...
        p = extract_pino_type(folder) or "unknown"
        if p == "PINO_1.0_finite" :
            base = f"PINO_Fixed"
        elif p == "PINO_2.0_finite":
            base = f"PINO_SoftAdapt"
        elif p == "PINO_3.0_finite":
            base = f"PINO_RelAdapt"
        else:
            base = f"PINO_{p}"
    else:
        base = folder
...
def plot_combined_logs_testing(logs):
    ...
    for folder, data in logs:
        ...
        #if any(mse): plt.plot(epochs, mse, label=f"Testing Loss: {label} - mse", color=color, linestyle="-.")
